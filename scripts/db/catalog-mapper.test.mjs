...
test("maps shared products and keeps tiered company prices separate", () => {
  ...
  assert.equal(products.find((item) => item.sourceKey === "led:module:lampro:p2").prices.diamond, 20);
...
test("creates every adjusted-brand module with Gold price above Lampro", () => {
  const products = mapStaticCatalog({
    ledCatalog: {
  ...
  assert.equal(products.find((item) => item.sourceKey === "led:module:absen:p1").prices.gold, 1150);
  assert.equal(products.find((item) => item.sourceKey === "led:module:absen:p2").prices.gold, 2150);
});
