...
function LoginPage(){const {user,login}=useAuth();const [show,setShow]=useState(false),[loading,setLoading]=useState(false),[error,setError]=useState("");useEffect(()=>{if(user)navigate("/admin/dashboard");},[user]);const submit=async(event)=>{event.preventDefault();setLoading(true);setError("");const f=new FormData(event.currentTarget);try{await login({email:f.get("email"),password:f.get("password"),remember:f.get("remember")==="on"});navigate("/admin/dashboard");}catch(e){setError(e.message);}finally{setLoading(false);}};return <main className="admin-login-page"><section className="admin-login-card"><div className="admin-login-brand"><span>C</span><div><h1>Calculator</h1><p>Administration</p></div></div><div className="admin-login-heading"><h2>Welcome back</h2><p>Sign in to manage catalogs, pricing, and company settings.</p></div><form onSubmit={submit}><label>Email<input name="email" type="email" autoComplete="username" required autoFocus/></label><label>Password<div className="admin-password-field"><input name="password" type={show?"text":"password"} autoComplete="current-password" required/><button type="button" onClick={()=>setShow((value)=>!value)} aria-label={show?"Hide password":"Show password"}>{show?"Hide":"Show"}</button></div></label><label className="admin-check"><input name="remember" type="checkbox"/>Keep me signed in on this device</label>{error?<div className="admin-login-error" role="alert">{error}</div>:null}<button className="admin-button primary full login-submit" disabled={loading}>{loading?"Signing in…":"Login"}</button></form><p className="admin-login-security">Protected administrative access</p></section></main>}
...
function ProtectedAdmin({path}){const {user,loading}=useAuth();const denied=!!user&&isPrivilegedAdminPath(path)&&!isPrivilegedAdmin(user);useEffect(()=>{if(!loading&&!user)navigate("/admin/login");else if(!loading&&denied)navigate("/admin/dashboard");},[denied,loading,user]);if(loading)return <div className="admin-loading">Loading Calculator admin…</div>;if(!user||denied)return null;return <CompanyProvider><AdminLayout path={path}>{resolvePage(path)}</AdminLayout></CompanyProvider>}
