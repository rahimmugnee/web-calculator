  ...
  return <div className={`dashboard-page${loading ? " is-loading" : ""}`}>
    ...
    <div className="dashboard-main-grid">
      <DashboardPanel className="dashboard-recent-panel" title={`Quotations · ${dateRange.monthName}`} action={<a href="/admin/quotations">View All</a>}>
        <div className="dashboard-table-scroll"><table className="dashboard-recent-table"><thead><tr><th>SL No.</th><th>Ref No.</th><th>Date</th><th>Client</th><th>Organization</th><th>Amount (BDT)</th><th>Created By</th><th>Status</th></tr></thead><tbody>{dashboard.recent.map((row,index) => <tr key={row.id}><td>{index+1}</td><td><b>{row.quotation_number}</b></td><td>{shortDate(row.created_at)}</td><td>{row.client_name || "—"}</td><td>{organization(row)}</td><td><b>{money(row.grand_total)}</b></td><td>{row.created_by_name || row.created_by || "Calculator"}</td><td><span className={`dashboard-status ${statusClass(row.status)}`}>{row.status}</span></td></tr>)}</tbody></table></div>
