...
const PAGE_SIZE=8;
const NO_BRAND_FILTER="__no_brand__";
const CABINET_SIZES={"640x480":{widthMm:640,heightMm:480,modulesPerCabinet:6},"640x640":{widthMm:640,heightMm:640,modulesPerCabinet:8},"960x960":{widthMm:960,heightMm:960,modulesPerCabinet:18},"1280x1280":{widthMm:1280,heightMm:1280,modulesPerCabinet:32}};
const CABINET_MATERIAL_LABELS={aluminium:"Aluminium",magnesium:"Magnesium",mild_steel:"Mild Steel"};
const CABINET_VARIANT_LABELS={open:"Open",backdoor:"Backdoor"};
...
export default function CategoriesPage(){
  ...
  const saveProduct=async(event)=>{
    ...
    try{
      const moduleCategory=selected.slug==="led-module",cabinetCategory=selected.slug==="cabinets";
      ...
      const metadata={
        ...
        ...(cabinetCategory?{
          displayType:cabinetLocation,
          materialCode:cabinetMaterial,
      ...
      const selectedBrandId=String(form.get("brand_id")||"").trim();
      const body={name:String(form.get("name")||"").trim(),model:String(form.get("model")||"").trim(),sku:editing?.sku||"",unit:String(form.get("unit")||"").trim()||"Pcs",category_id:selected.id,brand_id:selected.uses_brand&&selectedBrandId?Number(selectedBrandId):null,is_active:editing?.is_active!==false,technical_metadata:metadata};
    ...
    <div className="catalog-grid">
      ...
      <main className="catalog-center">
        {!selected?<section className="admin-panel"><EmptyState title="Select a category"/></section>:<>
	          <section className="category-summary admin-panel"><div className="category-summary-icon">{categoryIcon(selected.system_type)}</div><div className="category-summary-copy"><div><h2>{selected.name}{brandFilter?` / ${brandFilter===NO_BRAND_FILTER?"No Brand":assignedBrands.find((brand)=>String(brand.id)===String(brandFilter))?.name||""}`:""}</h2><StatusBadge active={selected.is_active}/></div><p><b>{systemLabels[selected.system_type]}</b><span>•</span>{selected.brand_count||0} Brands<span>•</span>{selected.model_count||0} Models</p><small>{canManage?"Manage brands and models under this component.":"Select a component from the category tree to manage its catalog."}</small></div><div className="category-summary-actions"><button className="admin-button secondary" type="button" onClick={()=>setModal({type:"category",category:selected})}>✎ Edit Category</button><button className="admin-icon-button" type="button" aria-label="More category actions">•••</button></div></section>
          <section className="catalog-main-card admin-panel">
            <div className="catalog-tabs">{tabs.map((item)=><button key={item} className={tab===item?"active":""} onClick={()=>setTab(item)} type="button">{item==="models"?`Models (${products.length})`:titleCase(item)}</button>)}{selected.uses_brand?<label className="catalog-brand-filter" aria-label="Filter by brand"><select value={brandFilter} onChange={(event)=>{setBrandFilter(event.target.value);setTab("models");}}><option value="">All Brands</option><option value={NO_BRAND_FILTER}>No Brand</option>{assignedBrands.map((brand)=><option key={brand.id} value={brand.id}>{brand.name}</option>)}</select></label>:null}{selected.slug==="led-module"?<label className="catalog-brand-filter catalog-technology-filter" aria-label="Filter by technology"><select value={technologyFilter} onChange={(event)=>{setTechnologyFilter(event.target.value);setTab("models");}}><option value="">All Technologies</option><option value="smd">SMD</option><option value="gob">GOB</option><option value="cob">COB</option></select></label>:null}<span className="catalog-tabs-spacer"/>{brandFilter&&brandFilter!==NO_BRAND_FILTER?<button className="quotation-delete-button" type="button" onClick={requestBrandDelete} title="Delete brand" aria-haspopup="dialog">Delete Brand</button>:null}</div>
...
function ProductModal({category,brands,product,priceReadOnly,onClose,onSave}){
  ...
  return <Modal title={product?.id?"Edit Model":"Add Model"} subtitle={`${systemLabels[category.system_type]} · ${category.name}`} onClose={onClose} wide><form onSubmit={onSave}><div className="admin-form-grid">
    ...
    {category.uses_brand?<label>Brand<select name="brand_id" defaultValue={product?.brand_id||""}><option value="">No Brand</option>{brands.map((brand)=><option key={brand.id} value={brand.id}>{brand.name}</option>)}</select></label>:null}
  ...
  </div><ModalActions onClose={onClose} label={product?.id?"Save Changes":"Add Model"}/></form></Modal>;
}
