...
export default function CategoriesPage(){
  ...
  const [modal,setModal]=useState(null),[deleteTarget,setDeleteTarget]=useState(null),[deleting,setDeleting]=useState(false),[notice,setNotice]=useState(""),[error,setError]=useState("");
  ...
  const filteredProducts=useMemo(()=>products.filter((product)=>{
    ...
    return matchesLocation&&matchesTechnology&&(!brandFilter||String(product.brand_id)===String(brandFilter))&&(!statusFilter||String(product.is_active)===statusFilter)&&(!query||[product.model,product.name,product.sku,product.unit].some((value)=>String(value||"").toLowerCase().includes(query)));
  ...
  useEffect(()=>{
    ...
    const closeOnEscape=(event)=>{if(event.key==="Escape"&&!deleting)setDeleteTarget(null);};
  ...
  const requestBrandDelete=()=>{
    const brand=assignedBrands.find((item)=>String(item.id)===String(brandFilter));
    if(brand)setDeleteTarget({kind:"brand",item:brand});
  };
  const saveProduct=async(event)=>{
    ...
    try{
      ...
      const body={name:String(form.get("name")||"").trim(),model:String(form.get("model")||"").trim(),sku:editing?.sku||"",unit:String(form.get("unit")||"").trim()||"Pcs",category_id:selected.id,brand_id:selected.uses_brand?Number(form.get("brand_id")):null,is_active:editing?.is_active!==false,technical_metadata:metadata};
  ...
  const deleteCatalogItem=async()=>{
    ...
    const {kind,item}=deleteTarget,name=kind==="brand"?item.name:(item.model||item.name||"this model");
    setDeleting(true);setError("");
    try{await remove(kind==="brand"?`/admin/brands/${item.id}`:`/admin/products/${item.id}`);if(kind==="brand")setBrandFilter("");setDeleteTarget(null);await refresh(`${name} moved to Recycle Bin.`);}catch(e){setError(e.message);}finally{setDeleting(false);}
  };
  ...
  return <div className="catalog-manager">
      ...
      <main className="catalog-center">
        {!selected?<section className="admin-panel"><EmptyState title="Select a category"/></section>:<>
          ...
          <section className="catalog-main-card admin-panel">
            <div className="catalog-tabs">{tabs.map((item)=><button key={item} className={tab===item?"active":""} onClick={()=>setTab(item)} type="button">{item==="models"?`Models (${products.length})`:titleCase(item)}</button>)}{selected.uses_brand?<label className="catalog-brand-filter" aria-label="Filter by brand"><select value={brandFilter} onChange={(event)=>{setBrandFilter(event.target.value);setTab("models");}}><option value="">All Brands</option>{assignedBrands.map((brand)=><option key={brand.id} value={brand.id}>{brand.name}</option>)}</select></label>:null}{selected.slug==="led-module"?<label className="catalog-brand-filter catalog-technology-filter" aria-label="Filter by technology"><select value={technologyFilter} onChange={(event)=>{setTechnologyFilter(event.target.value);setTab("models");}}><option value="">All Technologies</option><option value="smd">SMD</option><option value="gob">GOB</option><option value="cob">COB</option></select></label>:null}<span className="catalog-tabs-spacer"/>{brandFilter?<button className="quotation-delete-button" type="button" onClick={requestBrandDelete} title="Delete brand" aria-haspopup="dialog">Delete Brand</button>:null}</div>
    ...
    {deleteTarget?<div className="quotation-preview-backdrop quotation-delete-backdrop" onMouseDown={()=>{if(!deleting)setDeleteTarget(null);}}><section className="quotation-delete-confirm" role="dialog" aria-modal="true" aria-labelledby="catalog-delete-title" onMouseDown={(event)=>event.stopPropagation()}><div className="quotation-delete-icon" aria-hidden="true">!</div><h2 id="catalog-delete-title">Delete {deleteTarget.kind}?</h2><p>Are you sure you want to delete <strong>{deleteTarget.kind==="brand"?deleteTarget.item.name:(deleteTarget.item.model||deleteTarget.item.name)}</strong>? It will be moved to the Recycle Bin.</p><div className="quotation-delete-actions"><button type="button" className="quotation-delete-cancel" onClick={()=>setDeleteTarget(null)} disabled={deleting} autoFocus>Cancel</button><button type="button" className="quotation-delete-confirm-button" onClick={deleteCatalogItem} disabled={deleting}>{deleting?"Deleting...":`Delete ${titleCase(deleteTarget.kind)}`}</button></div></section></div>:null}
  </div>;
}
...
function ModelsSection({selected,products,total,allCount,modelSearch,setModelSearch,statusFilter,setStatusFilter,locationFilter,setLocationFilter,page,pages,setPage,onAddBrand,onAdd,onEdit,onDelete}){
  ...
  return <section className="models-section">{ledModules?<div className="model-location-tabs"><div className="model-location-options"><button type="button" className={locationFilter==="indoor"?"active":""} onClick={()=>setLocationFilter("indoor")}>Indoor Models</button><button type="button" className={locationFilter==="outdoor"?"active":""} onClick={()=>setLocationFilter("outdoor")}>Outdoor Models</button></div><div className="model-top-actions"><label className="compact-search">⌕<input value={modelSearch} onChange={(e)=>setModelSearch(e.target.value)} placeholder="Search model..." aria-label="Search model"/></label><select value={statusFilter} onChange={(e)=>setStatusFilter(e.target.value)} aria-label="Status filter"><option value="">All Statuses</option><option value="true">Active</option><option value="false">Inactive</option></select>{selected.uses_brand?<button className="admin-button secondary" type="button" onClick={onAddBrand}>＋ Add Brand</button>:null}<button className="admin-button primary" type="button" onClick={onAdd}>＋ Add Model</button></div></div>:<div className="models-heading"><h3>Models ({total})</h3><div><label className="compact-search">⌕<input value={modelSearch} onChange={(e)=>setModelSearch(e.target.value)} placeholder="Search model..." aria-label="Search model"/></label><select value={statusFilter} onChange={(e)=>setStatusFilter(e.target.value)} aria-label="Status filter"><option value="">All Statuses</option><option value="true">Active</option><option value="false">Inactive</option></select>{selected.uses_brand?<button className="admin-button secondary" type="button" onClick={onAddBrand}>＋ Add Brand</button>:null}<button className="admin-button primary" type="button" onClick={onAdd}>＋ Add Model</button></div></div>}
    {products.length?<div className="admin-table-wrap"><table className="admin-table catalog-model-table"><thead><tr><th>SL No.</th><th>Label</th><th>Model</th><th>Brand</th><th>Unit</th><th>Technology</th><th>Price</th><th aria-label="Actions"/></tr></thead><tbody>{products.map((product,index)=>{const price=calculatorModulePrice(product),model=product.model||product.name;return <tr key={product.id}><td>{(page-1)*PAGE_SIZE+index+1}</td><td>{product.name||"—"}</td><td><b>{model||"—"}</b></td><td>{product.brand_name||"—"}</td><td>{product.unit||"—"}</td><td>{String(product.technical_metadata?.technology||"—").toUpperCase()}</td><td>{price!==null?`৳ ${price.toLocaleString()}`:"—"}</td><td><div className="catalog-row-actions"><button className="row-action" type="button" onClick={()=>onEdit(product)} aria-label={`Edit ${model}`}>✎</button><button className="quotation-delete-button" type="button" onClick={()=>onDelete(product)} aria-label={`Delete ${model}`}>Delete</button></div></td></tr>;})}</tbody></table></div>:<EmptyState title="No models match these filters" description="Add a model or change the current filters."/>}
...
function ProductModal({category,brands,product,priceReadOnly,onClose,onSave}){const metadata=product?.technical_metadata||{};return <Modal title={product?.id?"Edit Model":"Add Model"} subtitle={`${systemLabels[category.system_type]} · ${category.name}`} onClose={onClose} wide><form onSubmit={onSave}><div className="admin-form-grid"><label>Label<input name="name" required defaultValue={product?.name||""}/></label><label>Model<input name="model" required defaultValue={product?.model||""} placeholder="Example: P1.53"/></label>{category.uses_brand?<label>Brand<select name="brand_id" required defaultValue={product?.brand_id||""}><option value="">Select brand</option>{brands.map((brand)=><option key={brand.id} value={brand.id}>{brand.name}</option>)}</select></label>:null}<label>Unit<input name="unit" required defaultValue={product?.unit||"Pcs"} placeholder="Example: Pcs"/></label><label>Technology<select name="technology" required defaultValue={String(metadata.technology||"smd").toLowerCase()}><option value="smd">SMD</option><option value="gob">GOB</option><option value="cob">COB</option></select></label><label>Default / Gold Price<input name="price_gold" type="number" min="0" step="0.01" required={!priceReadOnly} disabled={priceReadOnly} defaultValue={calculatorTierPrice(product,"default")??""}/>{priceReadOnly?<small>Calculated automatically from Mugnee pricing.</small>:null}</label></div><ModalActions onClose={onClose} label={product?.id?"Save Changes":"Add Model"}/></form></Modal>;}
