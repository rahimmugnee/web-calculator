...
const categoryComponentType = (category, requested) => requested || LED_COMPONENT_TYPES_BY_SLUG[category.slug] || category.system_type;
...
app.get("/api/public/company/:id/led-prices", asyncRoute(async (req, res) => {
  const pricing=await pricingProfile(idValue(req.params.id));
  const result=await pool.query(`SELECT p.source_key,p.source_catalog,p.component_type,p.name,p.model,p.unit,p.technical_metadata,
...
app.get("/api/admin/quotations/trash/:id", asyncRoute(async(req,res)=>{const id=idValue(req.params.id),companyId=req.query.companyId?idValue(req.query.companyId):null;const [quotation,items]=await Promise.all([pool.query("SELECT q.*,c.name company_name,creator.display_name created_by_name,creator.email created_by_email FROM quotations q JOIN companies c ON c.id=q.company_id LEFT JOIN users creator ON creator.id=q.created_by_user_id WHERE q.id=$1 AND q.deleted_at IS NOT NULL AND ($2::bigint IS NULL OR q.company_id=$2)",[id,companyId]),pool.query("SELECT * FROM quotation_items WHERE quotation_id=$1 ORDER BY line_number",[id])]);if(!quotation.rowCount)return res.status(404).json({error:"Deleted quotation not found."});res.json({...quotation.rows[0],items:items.rows});}));
